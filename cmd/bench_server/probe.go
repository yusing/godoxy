...
func probeWS(cfg probeConfig) (probeSample, error) {
	...
	if err != nil {
		...
		return probeSample{Status: status}, err
	}
	defer conn.Close()

	sample := probeSample{Dial: time.Since(start), Status: http.StatusSwitchingProtocols}
...
func printProbeSummary(samples []probeSample) {
