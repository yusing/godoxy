...
func (ep *Entrypoint) StartAddRoute(r types.Route) error {
	...
	case types.StreamRoute:
		if sniRoute, ok := asSNIPassthroughRoute(r); ok {
			if err := ep.sniPassthrough.AddRoute(sniRoute); err != nil {
				return err
			}
			ep.streamRoutes.Add(r)
			r.Task().OnCancel("remove_sni_passthrough_route", func() {
				ep.sniPassthrough.DelRoute(sniRoute)
				_ = r.Stream().Close()
