...
type (
	...
	HealthExtra struct {
		Config *loadbalancer.Config `json:"config"`
...
type (
	Status            = HealthStatus
	StatusString      = HealthStatusString
	Result            = HealthCheckResult
	Monitor           = HealthMonitor
	Checker           = HealthChecker
	MonCheck          = HealthMonCheck
	JSON              = HealthJSON
	JSONRepr          = HealthJSONRepr
	Extra             = HealthExtra
	Info              = HealthInfo
	InfoWithoutDetail = HealthInfoWithoutDetail
)
...
func (jsonRepr *HealthJSONRepr) MarshalJSON() ([]byte, error) {
	...
	return strutils.MarshalJSON(HealthJSON{
		Name:     jsonRepr.Name,
